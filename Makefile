...
QEMU_OPTS=-d cpu_reset,guest_errors -serial stdio -hda $(TEST_FAT) -drive if=none,id=stick,file=$(INITRD_TAR) -usb -device usb-mouse -device usb-storage,drive=stick -device usb-ehci,id=ehci -device usb-uhci,id=uhci
# QEMU_OPTS=-d cpu_reset,guest_errors -serial stdio -drive if=none,id=stick,file=$(TEST_FAT) -usb -device usb-mouse -device usb-storage,drive=stick
QEMU_MEM=12m
qemu: all .gdbinit
